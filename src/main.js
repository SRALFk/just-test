...
const M3_PALETTES = {
    orchid: {
        light: `--md-sys-color-primary:#A855F7;--md-sys-color-on-primary:#FFFFFF;--md-sys-color-primary-container:#FED7AA;--md-sys-color-on-primary-container:#7C2D12;--md-sys-color-secondary-container:#E0E7FF;--md-sys-color-on-secondary-container:#1E1B4B;--md-sys-color-tertiary-container:#FCE7F3;--md-sys-color-on-tertiary-container:#701A75;--md-sys-color-background:#F5F0FB;--md-sys-color-on-background:#1F003C;--md-sys-color-surface:#FFFFFF;--md-sys-color-on-surface:#120024;--md-sys-color-surface-variant:#F3E8FF;--md-sys-color-on-surface-variant:#6B21A8;`,
        dark: `--md-sys-color-primary:#C084FC;--md-sys-color-on-primary:#3B0764;--md-sys-color-primary-container:#C2410C;--md-sys-color-on-primary-container:#FFEDD5;--md-sys-color-secondary-container:#312E81;--md-sys-color-on-secondary-container:#E0E7FF;--md-sys-color-tertiary-container:#4A044E;--md-sys-color-on-tertiary-container:#FCE7F3;--md-sys-color-background:#0A0314;--md-sys-color-on-background:#F5E0FF;--md-sys-color-surface:#12091F;--md-sys-color-on-surface:#F3E8FF;--md-sys-color-surface-variant:#3B0764;--md-sys-color-on-surface-variant:#F5D0FE;`
    },
    olive: {
    ...
    lime: {
        light: `--md-sys-color-primary:#4D7C0F;--md-sys-color-on-primary:#FFFFFF;--md-sys-color-primary-container:#E9D5FF;--md-sys-color-on-primary-container:#3B0764;--md-sys-color-secondary-container:#FFEDD5;--md-sys-color-on-secondary-container:#7C2D12;--md-sys-color-tertiary-container:#F0FDF4;--md-sys-color-on-tertiary-container:#166534;--md-sys-color-background:#F7FEE7;--md-sys-color-on-background:#132000;--md-sys-color-surface:#FCFDF7;--md-sys-color-on-surface:#132000;--md-sys-color-surface-variant:#D1E897;--md-sys-color-on-surface-variant:#4C6A00;`,
        dark: `--md-sys-color-primary:#C5E85C;--md-sys-color-on-primary:#253600;--md-sys-color-primary-container:#3B0764;--md-sys-color-on-primary-container:#E9D5FF;--md-sys-color-secondary-container:#7C2D12;--md-sys-color-on-secondary-container:#FFEDD5;--md-sys-color-tertiary-container:#14532D;--md-sys-color-on-tertiary-container:#F0FDF4;--md-sys-color-background:#0A0E02;--md-sys-color-on-background:#E1FF85;--md-sys-color-surface:#0F1404;--md-sys-color-on-surface:#E1FF85;--md-sys-color-surface-variant:#3A5000;--md-sys-color-on-surface-variant:#C5E85C;`
    },
    sand: {
...
const DOM = {
    libView: document.getElementById('library-view'), readView: document.getElementById('reader-view'),
    mainHeader: document.getElementById('main-header'),
    grid: document.getElementById('book-grid'), empty: document.getElementById('empty-state'),
    topSection: document.getElementById('continue-reading-section'), topSlider: document.getElementById('top-books-slider'),
    load: document.getElementById('loading-state'), loadTxt: document.getElementById('loading-text'), loadBar: document.getElementById('loading-bar'), loadPct: document.getElementById('loading-percent'),
    ...
    inner: document.getElementById('reader-inner'), title: document.getElementById('reader-title'), count: document.getElementById('library-count'),
    tocPanel: document.getElementById('toc-panel'), tocList: document.getElementById('toc-list'),
    setPanel: document.getElementById('settings-panel'),
    readContent: document.getElementById('reader-content'), progBar: document.getElementById('reading-progress-bar'), progTxt: document.getElementById('reader-progress-text'),
    searchInput: document.getElementById('inbook-search-input'), searchRes: document.getElementById('search-results-panel'),
...
searchCapsule.addEventListener('click', (e) => {
    if (searchArea.classList.contains('search-active')) {
        if (e.target !== DOM.globalSearch) { closeSearch(false); }
...
let library = []; let activeBookId = null; let observer = null; let activePanel = null;
let activeOptsId = null; let currentSelection = { text: "", nodeIdx: -1 }; 
...
window.addEventListener('popstate', (e) => {
    ...
    else if (!DOM.readView.classList.contains('translate-y-full')) { _closeReaderAction(true); }
});
...
const applyLanguage = () => {
    ...
    setElementText('str-wel-backup', d.welBackup); document.getElementById('str-wel-backup-desc').innerHTML = d.welBackupDesc;
    setElementText('str-wel-format', d.welFormat); document.getElementById('str-wel-format-desc').innerHTML = d.welFormatDesc;
    ...
    if(document.getElementById('global-search')) document.getElementById('global-search').placeholder = d.searchBooks;
    if(document.getElementById('inbook-search-input')) document.getElementById('inbook-search-input').placeholder = d.searchPlaceholder;
    if(document.getElementById('note-input-text')) document.getElementById('note-input-text').placeholder = d.notePlaceholder;
    if(document.getElementById('library-count')) document.getElementById('library-count').textContent = `${(library.length)} ${d.booksCount}`;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if(savedModel && document.getElementById('gemini-model-select')) {
...
window.triggerSelectMode = function() {
    ...
    history.back(); 
    setTimeout(() => { toggleBatchDelete(false, targetId); }, 350); 
}
...
window.toggleBatchDelete = function(isFromHistory = false, initialSelectId = null) {
    ...
    if (!isBatchDeleteMode) {
        ...
        selectedForDelete = [];
        if (initialSelectId) selectedForDelete.push(initialSelectId);
    }
...
window.updateBatchSelectionUI = function() {
    ...
    const d = i18n[wikiLang] || i18n['id'];
    if(countEl) countEl.textContent = `${selectedForDelete.length} ${d.selected}`;
    ...
    showDialog(
        ...
        [
            ...
            { text: "Hapus", primary: true, action: async () => {
                window.closeDialog();
                const toDeleteSet = new Set(selectedForDelete);
                library = library.filter(b => !toDeleteSet.has(b.id));
...
function renderNodeText(text, annots) {
    if (!text) return "";
...
function applyThemeToDOM() {
    ...
    document.getElementById('dynamic-theme').innerHTML = `:root { ${rootVars} }`;
...
window.openModal = function(modalId, sheetId, isScale = false) {
    ...
    m.classList.remove('hidden'); requestAnimationFrame(() => { m.classList.remove('opacity-0'); if(isScale) { s.classList.remove('scale-75', 'translate-y-12'); } else { s.classList.remove('translate-y-full'); } });
}

window._closeModalAction = function(modalId, sheetId, isScale = false, isFromHistory = false) {
    if (!isFromHistory) { history.back(); return; }
    const m = document.getElementById(modalId); const s = document.getElementById(sheetId);
    if(isScale) { s.classList.add('scale-75', 'translate-y-12'); } else { s.classList.add('translate-y-full'); }
...
settingsSheet.addEventListener('touchend', (e) => {
    ...
    if (deltaY > 80) { 
        settingsSheet.style.transition = 'transform 0.4s cubic-bezier(0.34, 1.56, 0.64, 1)';
...
function syncTypoUI() {
    ...
    Object.values(maps.size).forEach(id => { document.getElementById(id).classList.remove('bg-m3-primary', 'text-m3-onPrimary'); document.getElementById(id).classList.add('text-m3-onSurfaceVariant'); });
    Object.values(maps.align).forEach(id => { document.getElementById(id).classList.remove('bg-m3-primary', 'text-m3-onPrimary'); document.getElementById(id).classList.add('text-m3-onSurfaceVariant'); });
    Object.values(maps.font).forEach(id => { document.getElementById(id).classList.remove('bg-m3-primaryContainer', 'text-m3-onPrimaryContainer'); });
...
async function loadLibrary() { try { library = await localforage.getItem('pdf_epub_master') || []; renderLibrary(); } catch (e) { console.error(e); } }
...
function createBookCard(book, isSlider = false, index = 0) {
    ...
    card.addEventListener('click', (e) => { 
        ...
        if (isBatchDeleteMode && !isSlider) {
            ...
            const idx = selectedForDelete.indexOf(book.id);
            if (idx > -1) {
                selectedForDelete.splice(idx, 1);
            } else {
                selectedForDelete.push(book.id);
            }
            updateBatchSelectionUI();
            return;
        }
        ...
        card.innerHTML = `
            ...
            <div class="relative z-10 flex flex-col h-full justify-between pointer-events-none border-none">
                ...
                <div class="mt-auto flex flex-col border-none">
                    <h3 class="font-bold text-sm leading-tight line-clamp-2 drop-shadow-md ${book.coverBase64 ? 'text-white' : ''}">${book.title}</h3>
                    <div class="w-full mt-2 border-none">
                        <div class="flex justify-between text-[0.65rem] font-bold opacity-90 mb-1 ${book.coverBase64 ? 'text-white' : ''}"><span>${progress}%</span></div>
                        <div class="h-1.5 w-full bg-black/20 dark:bg-white/20 rounded-full overflow-hidden border-none">
                            <div class="h-full ${book.coverBase64 ? 'bg-white' : 'bg-m3-primary dark:bg-m3-primaryContainer'} rounded-full border-none" style="width: ${progress}%"></div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        card.innerHTML = `
            ...
            <div class="relative z-10 flex flex-col h-full justify-between pointer-events-none border-none">
                ...
                <div class="mt-auto flex flex-col border-none">
                    ...
                    <h3 class="font-bold text-sm leading-tight mt-1 line-clamp-3 drop-shadow-md ${book.coverBase64 ? 'text-white' : ''}">${book.title}</h3>
                    ...
                    <div class="w-full border-none">
                        <div class="flex justify-between text-[0.6rem] font-bold opacity-90 mb-1 ${book.coverBase64 ? 'text-white' : ''}">
                            <span>${progress}%</span>
                        </div>
                        <div class="h-1.5 w-full bg-black/20 dark:bg-white/20 rounded-full overflow-hidden border-none">
                            <div class="h-full ${book.coverBase64 ? 'bg-white' : 'bg-m3-primary dark:bg-m3-primaryContainer'} rounded-full border-none" style="width: ${progress}%"></div>
...
function renderLibrary(filterText = "") {
    ...
    DOM.count.textContent = `${filteredLib.length} ${i18n[wikiLang].booksCount}`;
    ...
    showDialog(
        ...
        [
            ...
            { text: "Hapus", primary: true, action: async () => {
                ...
                await localforage.setItem('pdf_epub_master', library); 
                history.back(); setTimeout(() => renderLibrary(DOM.globalSearch.value), 350);
            }}
        ]
    );
};
    ...
    setTimeout(() => {
        ...
        book.nodes.forEach((node, i) => {
            ...
            if (node.tag === 'img') {
                ...
                el = document.createElement(node.tag); el.innerHTML = renderNodeText(node.text, annots); el.id = `node-${i}`;
...
function _closeSidePanelsAction(isFromHistory = false) { 
    if (!isFromHistory) { history.back(); return; }
    DOM.tocPanel.classList.add('translate-x-full', 'opacity-0'); DOM.setPanel.classList.add('translate-x-full', 'opacity-0'); 
    const overlay = document.getElementById('side-panel-overlay'); if(overlay) overlay.classList.add('hidden');
    activePanel = null;
    updateBottomNavUI(null);
}

function togglePanel(panelEl, name, btnId) { 
...
DOM.tocBtn.onclick = () => togglePanel(DOM.tocPanel, 'toc', 'btn-toc'); 
DOM.setBtn.onclick = () => togglePanel(DOM.setPanel, 'set', 'btn-settings');

let inbookSearchTimeout;
DOM.searchInput.addEventListener('input', (e) => {
    clearTimeout(inbookSearchTimeout);
    inbookSearchTimeout = setTimeout(() => executeSearch(), 300);
});

window.executeSearch = function() {
    const query = DOM.searchInput.value.toLowerCase().trim(); DOM.searchRes.innerHTML = '';
    const d = i18n[wikiLang] || i18n['id'];
    if(!query) { 
...
DOM.readContent.addEventListener('mousedown', (e) => { if(!window.getSelection().toString().trim()) { hideSelectionMenu(); } });
...
async function registerAnnotation(annotObj) {
    hideSelectionMenu(); const bookIndex = library.findIndex(b => b.id === activeBookId); if(bookIndex === -1) return;
    ...
    if(nodeEl && book.nodes[annotObj.nodeIdx]) {
        ...
        nodeEl.innerHTML = renderNodeText(book.nodes[annotObj.nodeIdx].text, currentAnnots);
    }
    window.getSelection().removeAllRanges();
}
...
window.setNoteColor = function(color) {
    ...
    ['yellow', 'green', 'pink'].forEach(c => {
        document.getElementById('nc-'+c).classList.remove('ring-m3-primary');
        document.getElementById('nc-'+c).classList.add('ring-transparent');
    });
    document.getElementById('nc-'+color).classList.add('ring-m3-primary');
    document.getElementById('nc-'+color).classList.remove('ring-transparent');
};
...
window.showAnnotationDetails = function(annotId) {
    ...
    setNoteColor(annot.color || 'yellow');
        ...
        if(bookIndex > -1) {
            ...
            if(annotIndex > -1) {
                ...
                localforage.setItem('pdf_epub_master', library).then(() => {
                    ...
                    if(nodeEl) nodeEl.innerHTML = renderNodeText(library[bookIndex].nodes[nodeIdx].text, library[bookIndex].annotations.filter(a => a.nodeIdx === nodeIdx));
                });
            }
        }
    } else {
    ...
    showDialog(
        ...
        [
            ...
            { text: "Hapus", primary: true, action: () => {
                window.closeDialog();
                activeAnnotationId = editingAnnotId;
...
window.deleteAnnotationById = async function() {
    if(!activeAnnotationId || !activeBookId) return; 
    const bookIndex = library.findIndex(b => b.id === activeBookId); if(bookIndex === -1) return;
    const book = library[bookIndex]; const annotIndex = book.annotations.findIndex(a => a.id === activeAnnotationId); if(annotIndex === -1) return;
    ...
    if(nodeEl && book.nodes[nodeIdx]) {
        ...
        nodeEl.innerHTML = renderNodeText(book.nodes[nodeIdx].text, currentAnnots);
    }
};

window.lookupDictionary = async function() {
    if(currentSelection.nodeIdx === -1) return; const term = currentSelection.text; hideSelectionMenu();
    const d = i18n[wikiLang] || i18n['id'];
    
    document.getElementById('ai-term').textContent = term;
    document.getElementById('ai-loading').classList.remove('hidden'); document.getElementById('ai-content').innerHTML = '';
    
    pushAppHistory(`ai-modal`);
    const m = document.getElementById('ai-modal'); const s = document.getElementById('ai-sheet');
    m.classList.remove('hidden'); requestAnimationFrame(() => { m.classList.remove('opacity-0'); s.classList.remove('translate-y-full', 'scale-75'); });
    ...
    try {
        ...
        window.getSelection().removeAllRanges(); document.getElementById('ai-loading').classList.add('hidden');
    }
};
...
window.closeAiModal = function(isFromHistory = false) {
    if (!isFromHistory) { history.back(); return; }
    const m = document.getElementById('ai-modal'); const s = document.getElementById('ai-sheet');
    s.classList.add('translate-y-full', 'scale-75'); m.classList.add('opacity-0'); 
    setTimeout(() => m.classList.add('hidden'), 300);
};

DOM.file.addEventListener('change', async (e) => {
    const file = e.target.files[0]; if (!file) return;
    const originalFilename = file.name; const ext = originalFilename.split('.').pop().toLowerCase(); const bookTitle = originalFilename.replace(/\.[^/.]+$/, "");
    DOM.load.classList.remove('hidden'); DOM.loadBar.style.width = '0%'; DOM.loadPct.textContent = '0%';
    ...
    finally { setTimeout(() => { DOM.load.classList.add('hidden'); DOM.file.value = ''; }, 500); }
});

async function handlePdf(file, bookTitle) {
    const d = i18n[wikiLang] || i18n['id'];
    DOM.loadTxt.textContent = "Opening PDF...";
    const arrayBuffer = await file.arrayBuffer(); const pdf = await pdfjsLib.getDocument({ data: arrayBuffer }).promise; const numPages = pdf.numPages;
    ...
    try {
        ...
        const page1 = await pdf.getPage(1); const viewport = page1.getViewport({ scale: 0.8 });
        const canvas = document.createElement('canvas'); const ctx = canvas.getContext('2d');
        canvas.height = viewport.height; canvas.width = viewport.width;
        await page1.render({ canvasContext: ctx, viewport: viewport }).promise; coverBase64 = canvas.toDataURL('image/jpeg', 0.6);
    } catch(e) {}

    for (let i = 1; i <= numPages; i++) {
        const pct = Math.round((i / numPages) * 100);
        if(i % 5 === 0 || i === numPages) await new Promise(r => setTimeout(r, 0));
        DOM.loadBar.style.width = `${pct}%`; DOM.loadPct.textContent = `${pct}%`; DOM.loadTxt.textContent = `${d.readingPage} ${i}`;

        const page = await pdf.getPage(i); const viewport = page.getViewport({ scale: 1.0 }); const pageHeight = viewport.height;
    ...
    DOM.loadTxt.textContent = d.formattingText; await new Promise(r => setTimeout(r, 50));
    ...
    allItems.forEach(item => {
        if (lastY !== -1 && Math.abs(item.y - lastY) > avgHeight * 1.5) {
            if (currentPar.trim() !== "") { 
                ...
                if (['.', '?', '!', '"', '”', '’', ':'].includes(lastChar)) { parsedNodes.push({ tag: 'p', text: currentPar.trim() }); currentPar = ""; } 
                else { currentPar += " "; }
            }
        }
    ...
    parsedNodes.forEach(curr => {
        if (cleanedNodes.length > 0) {
            ...
            if (curr.tag === prev.tag && (curr.tag === 'h1' || curr.tag === 'h2')) { prev.text += " " + curr.text; return; }
        }
    ...
    await localforage.setItem('pdf_epub_master', library); renderLibrary();
}

async function handleEpub(file, bookTitle) {
    const d = i18n[wikiLang] || i18n['id'];
    DOM.loadTxt.textContent = d.extractingEpub; DOM.loadBar.style.width = '10%'; DOM.loadPct.textContent = '10%';
    ...
    const rootfile = containerDom.querySelector("rootfile"); if (!rootfile) throw new Error("Invalid or corrupted EPUB file.");
    const opfPath = rootfile.getAttribute("full-path"); const basePath = opfPath.includes('/') ? opfPath.substring(0, opfPath.lastIndexOf('/') + 1) : '';

    DOM.loadTxt.textContent = d.analyzingStruct;
    const opfData = await zip.file(opfPath).async("string"); const opfDom = new DOMParser().parseFromString(opfData, "text/xml");
    const manifest = {}; opfDom.querySelectorAll("manifest > item").forEach(item => { manifest[item.getAttribute("id")] = item.getAttribute("href"); });
    const spine = []; opfDom.querySelectorAll("spine > itemref").forEach(ref => { spine.push(manifest[ref.getAttribute("idref")]); });

    let coverBase64 = null; const coverMeta = opfDom.querySelector("meta[name='cover']");
    if (coverMeta && manifest[coverMeta.getAttribute("content")]) {
        const coverPath = basePath + manifest[coverMeta.getAttribute("content")]; const coverFile = zip.file(coverPath);
        if (coverFile) { const base64Str = await coverFile.async("base64"); coverBase64 = "data:image/jpeg;base64," + base64Str; }
    }

    let parsedNodes = [];
    for (let i = 0; i < spine.length; i++) {
        const pct = Math.round(((i+1) / spine.length) * 80) + 20;
        DOM.loadBar.style.width = `${pct}%`; DOM.loadPct.textContent = `${pct}%`; DOM.loadTxt.textContent = `${d.extractingChapter} ${i+1}/${spine.length}`;
        await new Promise(r => setTimeout(r, 0));

        const htmlPath = basePath + spine[i]; const htmlFile = zip.file(htmlPath); if(!htmlFile) continue;
        const htmlString = await htmlFile.async("string"); const doc = new DOMParser().parseFromString(htmlString, "text/html");
        ...
        for (const el of elements) {
            ...
            if (tag === 'img' || tag === 'image') {
                ...
                if (src && !src.startsWith('http')) {
                    let absPath = resolveRelativePath(htmlPath, src); const imgFile = zip.file(absPath);
                    if (imgFile) { const b64 = await imgFile.async("base64"); parsedNodes.push({ tag: 'img', src: "data:image/jpeg;base64," + b64 }); }
                }
            } else {
                const text = el.textContent.trim();
                if (text) { let cleanTag = tag; if (tag === 'h3') cleanTag = 'h2'; parsedNodes.push({ tag: cleanTag, text: text }); }
            }
        }
    }
...
function resolveRelativePath(base, relative) {
    const stack = base.split('/'); stack.pop(); const parts = relative.split('/');
...
if ('serviceWorker' in navigator) {
    ...
    self.addEventListener('install', (e) => {
        ...
        e.waitUntil(caches.open(CACHE_NAME).then(c => c.addAll([
            '/', 'https://cdn.tailwindcss.com', 'https://cdnjs.cloudflare.com/ajax/libs/pdf.js/2.16.105/pdf.min.js', 'https://cdnjs.cloudflare.com/ajax/libs/pdf.js/2.16.105/pdf.worker.min.js', 'https://cdnjs.cloudflare.com/ajax/libs/localforage/1.10.0/localforage.min.js', 'https://cdnjs.cloudflare.com/ajax/libs/jszip/3.10.1/jszip.min.js', 'https://unpkg.com/lucide@latest'
        ])));
    });
